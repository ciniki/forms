...
// <rsp stat='ok' />
//
function ciniki_forms_checkAccess(&$ciniki, $tnid, $method) {
    //
    ...
    $rc = ciniki_tenants_checkModuleAccess($ciniki, $tnid, 'ciniki', 'forms');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    $modules = $rc['modules'];
    ...
    if( !isset($rc['ruleset']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.forms.1', 'msg'=>'No permissions granted'));
    }
    ...
    $strsql = "SELECT tnid, user_id "
        ...
        . "AND (permission_group = 'owners' "
            . "OR (permission_group = 'employees' AND modperms like '%\"ciniki.forms\"%') "
            . "OR permission_group = 'resellers') "
        . "";
    ciniki_core_loadMethod($ciniki, 'ciniki', 'core', 'private', 'dbHashQuery');
    $rc = ciniki_core_dbHashQuery($ciniki, $strsql, 'ciniki.tenants', 'user');
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.forms.2', 'msg'=>'Access denied.'));
    }
    //
    ...
    // By default fail
    //
    return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.forms.3', 'msg'=>'Access denied'));
}
?>
